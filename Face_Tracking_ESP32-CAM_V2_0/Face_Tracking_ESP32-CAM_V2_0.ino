...
/*------------------------------------------------------------------------------------------------------------------ 
  Face_Tracking_ESP32-CAM_V2_0 With ADS1115 Servo feedback                                                                   
------------------------------------------------------------------------------------------------------------------ 
...
{
  Serial.begin(115200);
  Wire.begin(15, 14); // SDA on GPIO15, SCL on GPIO14 (is this needed ???????????????????????????????????????????????????????????????????????????)
...
{
  display.setTextSize(2);
  display.setCursor(0, 0); display.print(Heading);
  display.display();
}
