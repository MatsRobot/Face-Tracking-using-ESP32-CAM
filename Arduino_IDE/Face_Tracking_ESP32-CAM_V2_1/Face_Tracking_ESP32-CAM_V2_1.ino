// #include <Arduino.h>
/*------------------------------------------------------------------------------------------------------------------ 
  Face_Tracking_ESP32-CAM_V2_1 
  With ADS1115 Servo feedback using Espressif version 1.0.4                                                                   
------------------------------------------------------------------------------------------------------------------ 
*/
/*------------------------------------------------------------------------------------------------------------------ 
PlatformIO comments:  
  Between version 1.0.4 and now, Espressif completely restructured their face detection libraries :

  The default "espressif32" platform is version 3.0, 6.0, or higher and this code requires 
  the file fd_forward.h which effectively doesn't exist in the same place anymore.

  1. PLATFORM VERSION: Fixed at espressif32@1.12.4 in platformio.ini to ensure compatibility with 
     legacy ESP-FACE (v1.0.4) headers (like fd_forward.h).
  2. FUNCTION PROTOTYPES: Unlike Arduino IDE, VS Code requires all functions to be declared 
     at the top of the file before they are called.
  3. TYPE MATCHING: Function return types in prototypes MUST match the actual definitions 
     (e.g., getMilliVolt must be consistently 'int' or 'float').
------------------------------------------------------------------------------------------------------------------ 
*/


/*
Bug fix since ver 2.0
  - SDA=14 and SCL=15
  - OLED address changed to normal 0x3C i.e. 0x78

 For Arduino IDE the Board AI Thinker ESP-CAM
...
As ESP32-CAM doesn't have dedicated i2c pins, TwoWire (TwoWire I2Cbus = TwoWire(0);) function is used on 15 (SDA) and 14 (SCL)
  OLED is back to I2C bus address 0x3C (normally this is 0x3C i.e. 0x78, ver 2.0 was changed to 0x3D i.e. 0x7A)
...
// ADD PROTOTYPES HERE:
void OLED_Heading(const char* text);
void OLED_Servo();
void InitialiseServos();
void MoveHead(int x, int y, int speed);
void MoveWaist(int y, int speed);
void OLED_XY(int x, int y);
void DisplayServoData();
int getMilliVolt(int pin);
void draw_face_boxes(dl_matrix3du_t *image_matrix, box_array_t *boxes);

// Note: Ensure the types (int, float, etc.) match your actual functions!
...
int MinTilt = 140;              // 0 Degree Minimum Tilt pulse width (Servo PWM 125-575)
int ZeroTilt = 200;             // 20 Degree Zero Tilt pulse width (Servo PWM 125-575)
int MaxTilt = 250;              // 55 Degree Maximum Tilt pulse width -- 110 steps (Servo PWM 125-575)
int MinPan = 190;               // 30 Degree Minimum Pan pulse width (Servo PWM 125-575)
int ZeroPan = 350;              // 90 Degree Zero Pan pulse width (Servo PWM 125-575)
int MaxPan = 510;               // 150 Degree Maximum Pan pulse width -- 320 steps (Servo PWM 125-575)
...
int OldPositionX = ZeroPan;     // Variable to hold previous Pan Pulse width (Servo PWM 125-575)
int OldPositionY = ZeroTilt;    // Variable to hold previous Tilt Pulse width (Servo PWM 125-575)
int OldWaistPosition = ZeroPan; // Variable to hold previous Waist Pulse width (Servo PWM 125-575)
int NoActivity = 0;             // variable to hold number of loops with no face detected
...
void setup()
{
  ...
  Wire.begin(14, 15); // SDA on GPIO14, SCL on GPIO15 (is this needed?)
  I2Cbus.begin(I2C_SDA, I2C_SCL, 100000); 
  delay(100); // Give the I2C hardware a moment to wake up

  if(!display.begin(SSD1306_SWITCHCAPVCC, SCREEN_ADDRESS)) {
      Serial.println(F("SSD1306 allocation failed"));
      // don't use while(true) yet so you can see Serial debugs
  }
  
  OLED_Heading("Head V2.1 ");  // Initialize I2C with our defined pins
...
// Settingup the ADS1115
  ...
  OLED_Heading("Head V2.1 ");

  // reset to zero position
  // Get the current position of the servos from the feedback and set it as the old position
  // Without this part, the software is not in control of the speed of servos when it first start
  // This section also requires calibration of the Vmin and Vmax which may be different for the servos
  // Input voltages to the servos also would change these values, so remmed out prior these calibrations
  ...
  InitialiseServos();  //initialise head to zero and move to max min on all servos
...
void draw_face_boxes(dl_matrix3du_t *image_matrix, box_array_t *boxes)
{
  ...
  fb_data_t fb;
  (void)fb; // Tells the compiler the variable is technically "used"
  ...
  for (i = 0; i < boxes->len; i++) 
    {
      ...
      half_width = w / 2;
      int face_center_pan = ScreenSizeX - x + half_width; // image frame face centre x co-ordinate
 
      y = (int)boxes->box[i].box_p[1];
      h = (int)boxes->box[i].box_p[3] - y + 1;
      half_height = h / 2;
      int face_center_tilt = ScreenSizeY- y + half_height;  // image frame face centre y co-ordinate
      ...
      // The coordinates start at the top right instead of bottom left, 
      OLED_XY(face_center_pan, face_center_tilt);
      MoveHead(face_center_pan, face_center_tilt, 10);
 ...
 void InitialiseServos()
{
  ...
  //Move head to middle position
  MoveHead(ScreenSizeX/2, ScreenSizeY/2, 10);
  MoveWaist (ScreenSizeX/2, 10);

  // display data in Serial.print
  Serial.print("Centre --- "); DisplayServoData();  // see what the servo voltages are for calibration of  MinV=0; and MaxV = 3332;

  MoveHead(ScreenSizeX/2, 0, 10);             //tilt head back - X=middle Y=0 back 
  MoveHead(ScreenSizeX/2, ScreenSizeY, 10);   //talt head forward
  MoveHead(ScreenSizeX/2, ScreenSizeY/2, 10); //talt head Zero

  MoveHead(0, ScreenSizeY/2, 5);              //Pan head Right
  MoveHead(ScreenSizeX, ScreenSizeY/2, 5);    //Pan head Left
  MoveHead(ScreenSizeX/2, ScreenSizeY/2, 5);  //Pan head Zero

  MoveWaist(0, 10);                           //Pan Waist Right
  MoveWaist(ScreenSizeX, 10);                 //Pan Waist Left
  MoveWaist(ScreenSizeX/2, 10);               //Pan Waist Zero

  Serial.print("Centre Position - ");  DisplayServoData();
...
------------------------------------------------------------------------------------------------------------------ */
// THE DEFINITION (at the bottom)
void OLED_Heading(const char* text) {  // <-- Ensure "const char*" is used here
  display.clearDisplay();
  display.setTextSize(2);
...
void OLED_Servo()
{
    ...
    display.setCursor(0, 20); display.print("P:     T:     W:    ");
    display.setCursor(10, 20); display.print(int(getMilliVolt(0))); 
    display.setCursor(52, 20); display.print(int(getMilliVolt(1))); 
    display.setCursor(94, 20); display.print(int(getMilliVolt(2)));
    display.setTextColor(WHITE, BLACK);
    display.display();
}

// Third line showing the commanded/actual servo positions in PWM
void OLED_CurrentPos()
{
    display.setTextSize(1);
    display.setCursor(0, 31); display.print("                    ");
    display.setCursor(10, 31); display.print(NewPositionX); 
    display.setCursor(52, 31); display.print(NewPositionY); 
    display.setCursor(94, 31); display.print(NewWaistPosition);
    display.setTextColor(WHITE, BLACK);
    display.display();
}
...
void OLED_XY(int CameraX, int CameraY)
{
    ...
    display.setCursor(10, 50); display.print(":"); display.setCursor(65, 50); display.print("Y"); display.setCursor(75, 50); display.print(":"); 
    display.setCursor(19, 50); display.print(CameraX); 
    display.setCursor(85, 50); display.print(CameraY); 
    display.setTextColor(WHITE, BLACK);
    display.display();
}
...
------------------------------------------------------------------------------------------------------------------ */
 void MoveHead(int ScreenPosX, int ScreenPosY, int speed)
 {
  ...
  NewPositionX = map(ScreenPosX, 0, ScreenSizeX, MinPan, MaxPan);    //NewPositionX is in PWM between MinPan and MaxPan
  NewPositionY = map(ScreenPosY, 0, ScreenSizeY, MinTilt, MaxTilt);  //NewPositionY is in PWM between MinTilt and MaxTilt

  OLED_Servo();       //Display the current servo voltage feedback in mV
  OLED_CurrentPos();  //Display the Actual servo position in PWM

  // New Routine to move diagnal
  if (NewPositionX > OldPositionX)
  {
    for(pos = OldPositionX; pos < NewPositionX; pos++) // in steps of 1 PWM
      {  
        pwm.setPWM(0,0,pos);
        ...
        delay(speed);             // mini delay for each step in milli sec (default is 10)
      }
    delay(100);
  }
...
// ensure that the Y position is complete
  if (NewPositionY > OldPositionY)
    {
      for(pos = OldPositionY; pos < NewPositionY; pos ++) // in steps of 1 PWM
        {  
...
------------------------------------------------------------------------------------------------------------------ */
int getMilliVolt(int Channel) {return readValue(Channel + 4);}//return the voltage in mV
int getAngle(int Channel) {return map (readValue(Channel + 4), MinV,MaxV, 0,360);} //return the angle in degrees
int getPWM(int Channel) {return map (readValue(Channel + 4), MinV,MaxV, SERVOMIN,SERVOMAX);} //return the angle in degrees
...
void DisplayServoData()
{
  Serial.print("Servo - Min PWM: "); Serial.print(SERVOMIN); Serial.print(" Max PWM: "); Serial.println(SERVOMAX);    
  Serial.print("Pan: "); Serial.print(getPWM(0)); Serial.print("PWM V: "); Serial.print(getMilliVolt(0));     
  Serial.print("mV  Tilt: "); Serial.print(getPWM(1)); Serial.print("PWM V: "); Serial.print(getMilliVolt(1));  
  Serial.print("mV  Waist: "); Serial.print(getPWM(2)); Serial.print("PWM V: "); Serial.print(getMilliVolt(2));  
  Serial.println("mV");    
}
